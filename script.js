...
(function () {
    ...
    // Sync Geofence safely without causing input cursor jumping
    ...
    const saveSettings = () => { 
        ...
        if(DOM.geofenceRadiusInput && (STATE.role === 'admin' || STATE.role === 'creator')) {
            ...
            STATE.geofenceRadius = rad; // <-- Updates map instantly before server replies!
            socket.emit('updateGeofenceRadius', rad);
            
            // Helpful warning if admin hasn't injected yet to set map center
            const adminNode = STATE.group.find(p => p.role === 'admin' || p.role === 'creator');
            if (!adminNode || adminNode.lat === null) {
    ...
    function renderRoster() {
        ...
        DOM.rosterList.innerHTML = STATE.group.map(p => {
            ...
            // New Visual Status Rule based exactly on Datum Zero Line
            const tolerance = 0.1;
            let statusClass = "status-within"; // Cyan (On Line)
            if (relHeight > tolerance) statusClass = "status-above"; // Green
            else if (relHeight < -tolerance) statusClass = "status-below"; // Red
    ...
    function renderGraph() {
        ...
        for (let f = startFloor; f <= endFloor; f++) {
            ...
            if (yFloor >= paddingTopBottom && yFloor <= H - paddingTopBottom) {
                if (f !== 0) { // Don't overwrite the main datum 0 line
