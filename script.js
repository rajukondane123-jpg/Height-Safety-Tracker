/* ============================================================
   ALTIGUARD: MASTER ENGINE
   Group tracking + Weather + Hospitals + Browser Ntfy Push
   ============================================================ */
...
(function () {
  ...
  // Universal API Timeout Wrapper to prevent browser freezing
  ...
  // When ANOTHER device detects a fall
  ...
  function syncToServer() {
    if (isSocketConnected) socket.emit('updateGroup', group);
  }

  // Safely auto-builds your new UI (including your developer signature!)
  function ensureNewUIElements() {
    try {
      // 1. Inject Summary Bar
        ...
        if (main) {
          ...
          bar.innerHTML = `
            ...
            <div class="stat-box">
              ...
              <strong id="statStatus" class="status-secure">🟢 CONNECTING...</strong>
            </div>
          `;
      ...
      // 3. INJECT DEVELOPER CREDIT SIGNATURE
      ...
      console.warn("UI Injection skipped to preserve layout");
    }
  }
  ...
  // =========================================================
  // THE PERFECT NTFY TRIGGER (Direct from Browser)
  // =========================================================
  function triggerAlert(person, dropAmount, isTest) {
    ...
    if (isSocketConnected) socket.emit('triggerAlert', entry);
    ...
    // 100% Client-Side Push Notification
    if (activeTopic) {
      const cleanTopic = activeTopic.replace(/[^a-zA-Z0-9-_]/g, "");
      const workerInfo = entry.phone ? `${entry.name} (${entry.phone})` : entry.name;
      const message = `Worker ${workerInfo} dropped ${entry.drop}m. Check status immediately.`;
      ...
      if (entry.lat && entry.lon) {
        headers['Click'] = `https://www.google.com/maps?q=${entry.lat},${entry.lon}`;
      }

      fetchWithTimeout(`https://ntfy.sh/${cleanTopic}`, {
        method: 'POST',
        body: message,
        headers: headers,
        timeout: 5000 
      ...
      .then(res => {
         if (res.ok) console.log(`✅ Ntfy push sent successfully! Status: ${res.status}`);
         else console.error(`⚠️ Ntfy rejected the push. Status: ${res.status}`);
      })
      .catch(err => console.error("❌ Browser Network Error / Adblocker blocked Ntfy:", err));
    }
  }
  ...
  function renderSummary() {
    try {
      ...
      if (!elActive || !elHighest || !elStatus) return;
  ...
  function initMap() {
    ...
    if (!mapEl || typeof L === "undefined") {
      console.warn("Leaflet Map unavailable. Operating in Graph-Only mode.");
      return;
    }
    ...
    try {
      ...
      console.error("Map rendering skipped safely.", e);
    }
  }
  ...
  function checkWeather() {
    ...
    fetchWithTimeout(`https://api.open-meteo.com/v1/forecast?latitude=${person.lat}&longitude=${person.lon}&current_weather=true`, { timeout: 6000 })
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => console.warn("Weather skipped."));
  }

  function fetchHospitals() {
    if (!map || map.getZoom() < 11 || typeof L === "undefined") return; 
  ...
  function init() {
    ...
    if (setRefBtn) {
      setRefBtn.addEventListener("click", () => {
        ...
        if (!isNaN(val)) {
          globalReference = val;
          if (isSocketConnected) socket.emit('updateReference', val);
          render();
        }
      });
    }
    ...
    if (testAlertBtn) {
      testAlertBtn.addEventListener("click", () => { triggerAlert(null, 1.80, true); });
    }
