...
io.on('connection', (socket) => {
  ...
  socket.on('triggerAlert', (alertData) => {
    ...
    if (!topic) {
      console.log("Ntfy Error: No topic specified in settings.");
      return;
    }
    ...
    const req = https.request(options, (res) => {
      console.log(`Ntfy notification sent successfully. Status code: ${res.statusCode}`);
    });

    req.on('error', (error) => {
      console.error('Ntfy Dispatch Error:', error);
    });
